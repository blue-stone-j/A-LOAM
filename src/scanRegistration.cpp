...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg)
{
  ...
  // 不是每个激光点都能够返回并被接收，例如射向天空的点。这些激光点就是nan点（可以视为不存在的点，但在原始的点云数据中却占据位置），需要去除
    ...
    for (int j = 0; j < 6; j++)
    {
      ...
      for (int k = ep; k >= ep; k--) // 倒序遍历是为了从曲率最大处挑选
      {
        ...
        if (cloudNeighborPicked[ind] == 0 && cloudCurvature[ind] > 0.1)
        {
          ...
          else if (largestPickedNum <= 20) // 20个曲率稍微大一些的点
          {
          ...
          // 为保证特征点不集中，选中的特征点周围的点避免选取。该周围指的是物理空间意义上的周围，不是存储空间意义上的周围。
          // 从该点向右判断5个点
          for (int l = 1; l <= 5; l++)
          {
  ...
  if (PUB_EACH_LINE)
  {
    for (int i = 0; i < N_SCANS; i++)
    {
      ...
      scanMsg.header.frame_id = "/camera_init"; // 原代码中以相机作为坐标系原点而不是激光雷达
...
int main(int argc, char **argv)
{
  ...
  // 从参数服务器获取参数；参数可能写在launch文件中；此处为从配置文件中获取激光雷达的线数，默认为16
